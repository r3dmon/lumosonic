 ...
 */
import { getPDF as api_web_showcase_getPDF } from "~backend//showcase";
import { renderPage as api_web_ssr_renderPage } from "~backend//ssr";
import {
    serveIndex as api_web_static_serveIndex,
    serveStatic as api_web_static_serveStatic
} from "~backend//static";
import { submit as api_web_submit_submit } from "~backend//submit";
...
export namespace web {
    ...
    export class ServiceClient {
        ...
        public async getPDF(): Promise<ResponseType<typeof api_web_showcase_getPDF>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/api/showcase/pdf`, {method: "GET", body: undefined})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_web_showcase_getPDF>
        }

        /**
         * Serves the SSR HTML for the frontend application
         */
        public async renderPage(params: { path: string[] }): Promise<ResponseType<typeof api_web_ssr_renderPage>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/ssr/${params.path.map(encodeURIComponent).join("/")}`, {method: "GET", body: undefined})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_web_ssr_renderPage>
        }

        /**
         * Serves the main index.html for SPA fallback
         */
        public async serveIndex(): Promise<ResponseType<typeof api_web_static_serveIndex>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/`, {method: "GET", body: undefined})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_web_static_serveIndex>
        }

        /**
         * Serves static assets from the frontend build
         */
        public async serveStatic(params: { path: string[] }): Promise<ResponseType<typeof api_web_static_serveStatic>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/assets/${params.path.map(encodeURIComponent).join("/")}`, {method: "GET", body: undefined})
